...
clean_net_survey <- NETTING_INFO_semiclean_TSM_v2 %>% 
...
net_species_table <- SURVEY_SPP_semiclean_TSM_v2 %>% 
  mutate(across(where(is.character), tolower)) %>% 
  rename(survey_id = assmt_code,
         seine_id = seine_code) %>% 
  select(!c(site_code, survey_code, year, date))

#missing survey_ids in clean_net_survey - seems to be no missing survey_ids or sein_ids
missing_survid_netsur <- net_species_table %>% 
  filter(!seine_id %in% c(net_species_table$seine_id))
